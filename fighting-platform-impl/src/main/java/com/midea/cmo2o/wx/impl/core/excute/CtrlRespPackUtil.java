...
import com.midea.cmo2o.wx.impl.utils.JacksonMapper;
...
//	private static MessageProducer messageProducer;
	...
	* @return BodyData 返回类型 
	* @throws
	 */
	public static BodyData execInvokeService(BaseRequest request,
			BodyData response, ControllerWrapper wrapper) {
	...
	//包装Response
	private static void packCoreResponse(BodyData response,
			String resultCode, String errorDetail) {
	...
	public static boolean isTimeoutThrowable(Throwable cause){
		if(cause != null && (cause instanceof TimeoutException
				|| cause instanceof HessianConnectionException
	...
	* @param @param errorStr
	...
	/*public static boolean isPrintDebugLog(String facadeName){
		List<String> facadeList = ConfigCallback.getFacadeList();
		if(facadeList != null && facadeList.contains(facadeName)){
			return true;
		}else{
			return false;
		}
